...
async function publishToInstagram(post) {
  ...
  // Instagram butuh waktu buat "memproses" container (unduh & validasi
  // gambar) sebelum boleh di-publish — kalau langsung publish, dia balas
  // error 9007 "media is not ready". Jadi kita cek status_code-nya dulu,
  // sampai FINISHED, dengan budget waktu terbatas (function ini punya
  // waktu sendiri, terpisah dari proses generate konten).
  const maxAttempts = 8;
  const delayMs = 1200;
  let status = null;
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (status === 'ERROR') {
      throw new Error(`Container gagal diproses Instagram: ${JSON.stringify(statusData).slice(0, 300)}`);
    }
    await new Promise((resolve) => setTimeout(resolve, delayMs));
  }
  if (status !== 'FINISHED') {
    throw new Error(`Container belum FINISHED setelah ditunggu (status terakhir: ${status}). Coba publish manual lagi nanti.`);
  }
